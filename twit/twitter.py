...
class TwitterListResponse(list):
    def __init__(self, data, seq = ()):
        super(TwitterListResponse, self).__init__(seq)
        self.data = data

    def config(self, method, *args, **kwargs):
        self.method = method
        self.args = args
        self.kwargs = kwargs
    ...
    def next(self):
        if not hasattr(self, 'method'):
            raise Exception('Response Not Configured')
        kwargs = {}
        kwargs.update(self.kwargs)
        if self.has_cursor:
            kwargs['cursor'] = self.data.get('next_cursor_str')
        return self.method(*self.args, **kwargs)

    def has_cursor(self):
        return isinstance(self.data, dict) and self.data.has_key('next_cursor_str')

    @property
    def cursor(self):
        if self.has_cursor:
            return self.data.get('next_cursor_str')
        return -1
...
class Api(object):
    ...
    def GetPublicTimeline(self,
        ...
        return TwitterListResponse(data, [Status.NewFromJsonDict(x) for x in data])
    ...
    def GetRetweets(self, statusid):
        ...
        data = simplejson.loads(json)
        self._CheckForTwitterError(data)
        return TwitterListResponse(data, [Status.NewFromJsonDict(s) for s in data])
    ...
    def GetFriends(self, user=None, cursor=-1):
        ...
        data = simplejson.loads(json)
        self._CheckForTwitterError(data)
        response = TwitterListResponse(data, [User.NewFromJsonDict(x) for x in data['users']])
        response.config(self.GetFriends, **{'user':user})
        return response

    def GetAllFriends(self, user = None):
        friends = []
        response = self.GetFriends(user)
        friends += response
        while int(response.cursor):
    ...
    def GetFollowers(self, page=None):
        ...
        data = simplejson.loads(json)
        self._CheckForTwitterError(data)
        response = TwitterListResponse(data, [User.NewFromJsonDict(x) for x in data])
    ...
    def GetFeatured(self):
        ...
        data = simplejson.loads(json)
        self._CheckForTwitterError(data)
        return TwitterListResponse(data, [User.NewFromJsonDict(x) for x in data])
    ...
    def UsersLookup(self, user_id=None, screen_name=None, users=None):
        ...
        data = simplejson.loads(json)
        self._CheckForTwitterError(data)
        return TwitterListResponse(data, [User.NewFromJsonDict(u) for u in data])
    ...
    def GetDirectMessages(self, since=None, since_id=None, page=None):
        ...
        data = simplejson.loads(json)
        self._CheckForTwitterError(data)
        return TwitterListResponse(data, [DirectMessage.NewFromJsonDict(x) for x in data])
    ...
    def GetSubscriptions(self, user, cursor=-1):
        ...
        data = simplejson.loads(json)
        self._CheckForTwitterError(data)
        return TwitterListResponse(data, [List.NewFromJsonDict(x) for x in data['lists']])

    def GetSubscribers(self, user, list, cursor=-1):
        ...
        Args:
            user:
                The twitter name or id of the user
            list:
                The list you want to find the users of
            cursor:
                "page" value that Twitter will use to start building the
                list sequence from.    -1 to start at the beginning.
                Twitter will return in the result the values for next_cursor
        ...
        url = '%s/%s/%s/subscribers.json' % (self.base_url, user, list)
        parameters = {}
        parameters['cursor'] = cursor
        ...
        data = simplejson.loads(json)
        self._CheckForTwitterError(data)
        response = TwitterListResponse(data, [List.NewFromJsonDict(x) for x in data['lists']])
        response.config(self.GetSubscribers, user, list)
        return response
    ...
    def GetList(self, user, id):
        '''Show the specified list.
            ...
            Args:
                user:
                    Required. The user who own's the list
                id:
                    Required. The id or slug of the list.
            Returns:
                A List
            '''
        if not self._oauth_consumer:
    ...
    def GetListMembers(self, uri, cursor = -1):
        ...
        Args:
            uri:
                Required. the uri of the list (example: /username/list_slug.
            cursor:
                "page" value that Twitter will use to start building the
                list sequence from.    -1 to start at the beginning.
                Twitter will return in the result the values for next_cursor
                and previous_cursor. [Optional]
        Returns:
            A sequence of twitter.User instances, one for each member
        '''
        if not self._oauth_consumer:
            raise TwitterError("twitter.Api instance must be authenticated")
        if uri[0] != '/':
            uri = '/' + uri
        url = '%s%s/members.json' % (self.base_url, uri)
        parameters = {}
        if cursor:
            parameters['cursor'] = cursor
        json = self._FetchUrl(url, parameters=parameters)
        ...
        self._CheckForTwitterError(data)
        response = TwitterListResponse(data, [User.NewFromJsonDict(x) for x in data])
        response.config(self.GetListMembers, cursor=cursor)
        return response
